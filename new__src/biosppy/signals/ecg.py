...
def __getRR(signal=None, rpeaks=None, sampling_rate=1000., resample_factor=0.7):
    """Extract RR segment templates from an ECG signal, given a list of
    R-peak locations.
    ...
    for i in range(R.__len__()-1):
        ...
        if b > length:
            break
        temp = signal[a:b]
        temp = scipy.signal.resample(temp, int(resample_factor * sampling_rate))
...
def extract_heartbeats(signal=None, rpeaks=None, sampling_rate=1000.,
                       before=0.2, after=0.4, segmentation_method="window"):
    """Extract heartbeat templates from an ECG signal, given a list of
    R-peak locations.
    ...
    if segmentation_method == "window":
        ...
        templates, newR = __getRR(signal=signal, rpeaks=rpeaks, sampling_rate=sampling_rate)
        return utils.ReturnTuple((templates, newR), ('templates', 'rpeaks'))
...
def getQPositions(ecg_proc=None, print_samples=False):
    template_r_position = 100
    Q_positions = []
    Q_start_positions = []
...
def getSPositions(ecg_proc=None, print_samples=False):
    template_r_position = 100
    S_positions = []
    S_end_positions = []
...
def getPPositions(ecg_proc=None, print_samples=False):
...
def getTPositions(ecg_proc=None, print_samples=False):
    template_r_position = 100 #R peek on the template is always on 100 index
